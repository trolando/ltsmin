...
rel_create_rw(vdom_t dom, int r_k, int *r_proj, int w_k, int *w_proj)
{
    ...
    /*
    printf("r_proj:");
    for (int i=0;i<r_k;i++) printf(" %d", r_proj[i]);
    printf("\n");
    printf("w_proj:");
    for (int i=0;i<w_k;i++) printf(" %d", w_proj[i]);
    printf("\n");
    printf("a_proj:");
    for (int i=0;i<a_k;i++) printf(" %d", a_proj[i]);
    printf("\n");
    printf("ro_proj:");
    for (int i=0;i<ro_k;i++) printf(" %d", ro_proj[i]);
    printf("\n");
    */
...
set_enum_match(vset_t set, int p_len, int* proj, int* match, vset_element_cb cb, void* context) 
{
    ...
    for (int i=p_len;i-->0;) {
        uint32_t b = match[i];
        for (int j=statebits;j-->0;) {
            BDD val = sylvan_ithvar(set->dom->vec_to_bddvar[proj[i]*statebits+j]);
...
set_copy_match(vset_t dst, vset_t src, int p_len, int* proj, int*match)
{
    ...
    for (int i=p_len;i-->0;) {
        uint32_t b = match[i];
        for (int j=statebits;j-->0;) {
            BDD val = sylvan_ithvar(src->dom->vec_to_bddvar[proj[i]*statebits+j]);
            if (!(b&1)) val = sylvan_not(val);
            match_bdd = sylvan_and(val, match_bdd);
            b >>= 1;
        }
...
vdom_create_sylvan(int n)
{
    ...
    // Create universe
    dom->vec_to_bddvar = (BDDVAR*)RTmalloc(sizeof(BDDVAR) * statebits * n);
    dom->prime_vec_to_bddvar = (BDDVAR*)RTmalloc(sizeof(BDDVAR) * statebits * n);
    for (int i=0; i<n; i++) {
        for (int j=0; j<statebits; j++) {
            dom->vec_to_bddvar[i*statebits+j] = 2*(i*statebits+j);
            dom->prime_vec_to_bddvar[i*statebits+j] = 2*(i*statebits+j)+1;
        }
    }

    dom->state_variables = sylvan_ref(sylvan_set_fromarray(dom->vec_to_bddvar, statebits * n));
    dom->prime_variables = sylvan_ref(sylvan_set_fromarray(dom->prime_vec_to_bddvar, statebits * n));

    // Create action labels
    BDDVAR vars[actionbits];
    for (int i=0; i<actionbits; i++) vars[i] = 1000000+i;
    dom->action_variables = sylvan_ref(sylvan_set_fromarray(vars, actionbits));
